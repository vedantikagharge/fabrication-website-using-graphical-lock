...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("SELECT image1, image2, image3 FROM admin_users WHERE username=?");
    ...
    foreach ($images as $image) {
        ...
        if (move_uploaded_file($image, $temp_file)) {
            $temp_images[] = $temp_file;
        } else {
            $_SESSION['message'] = "Error: Unable to move uploaded file.";
    ...
    if (count($temp_images) !== 3) {
        $_SESSION['message'] = "Error: Unable to create temporary files.";
        header("Location: request.php");
        exit();
    }
...
<body>
    ...
    <form action="request.php" method="post" enctype="multipart/form-data">
        ...
        <p>Don't have an account? <a href="request_1.php">Signup</a></p>
    </form>
</body>
</html>
